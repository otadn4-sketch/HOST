...
نسخه ۱.۵.۲: آرشیو فایل و ثبت دستی ارتباطات. ارسال سنتی در پیام‌رسان‌ها خارج از سامانه انجام می‌شود؛ سامانه فقط ثبت می‌کند چه فایلی، به چه مخاطبی، در چه تاریخی، با چه هدفی، از چه کانالی و با چه یادداشتی.
...
روی ویندوز: `.\scripts\reset-data.cmd --dry-run` سپس همان سوئیچ حذف. `.env` و گواهی TLS نگه داشته می‌شوند. `bootstrap.sh` را برای ریست اجرا نکنید.
...
بکاپ رمزنگاری‌شدهٔ جدا برای دیتابیس و vault: `./scripts/backup.sh`. بازیابی کنترل‌شده: `./scripts/restore.sh` (به vault زنده نمی‌نویسد).
