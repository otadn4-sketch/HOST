# سامانه اشتراک‌گذاری امن فایل و گزارش ایتان

نسخه ۱.۱.۰: پرتال کاربری، گفت‌وگو با منابع، خلاصه‌سازی هوشمند، مدیریت واحد و نقش سازمانی، و پنهان‌سازی داده امنیتی از غیرمدیران. نصب به‌روزرسانی روی سرور موجود از طریق بسته zip امضاشده در «تنظیمات ← به‌روزرسانی سامانه» (راهنما: `docs/release-bundles.md`).

نسخهٔ قابل‌استقرار و خودمیزبان برای شبکه ایتان. رابط فارسی/RTL حفظ شده و به API واقعی FastAPI متصل است. هیچ سرویس ابری جانبی، Firebase، S3 یا telemetry بیرونی استفاده نمی‌شود. هوش مصنوعی به‌صورت پیش‌فرض با دستیار محلی و دو پرامپت سازمانی کار می‌کند.
