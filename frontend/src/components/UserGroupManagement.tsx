...
import { User, Department, UserRole, UserStatus, OrgRole } from '../types';
import { StorageService } from '../services/storageService';
import { DataApi, mapRole } from '../services/api';
...
export const UserGroupManagement: React.FC<UserGroupManagementProps> = ({
  ...
  const [roleLevel, setRoleLevel] = useState<UserRole>('user');
  ...
  useEffect(() => {
    DataApi.roles().then((r) => setRoles((r.roles || []).map(mapRole))).catch(() => setRoles([]));
  }, [activeTab]);
  ...
  const openEditUser = (user: User) => {
    ...
    setEditOrgRoleId(user.orgRoleId || '');
  };

  const handleEditUser = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!editingUser) return;
    await DataApi.patchUser(editingUser.id, {
      full_name: editFullName,
      email: editEmail,
      role: editRole,
      group_id: editDeptId,
      org_role_id: editOrgRoleId || null,
    });
    setEditingUser(null);
    onRefresh();
  };
  ...
  const handleDeleteUser = async (user: User) => {
    if (!window.confirm(`حذف کاربر «${user.fullName}»؟`)) return;
    await DataApi.deleteUser(user.id);
    onRefresh();
  };
  ...
  const openEditDept = (dept: Department) => {
    ...
    setNewDeptMaxSize(dept.maxFileSizeMB);
    setNewDeptManager(dept.managerName);
  };

  const handleEditDepartment = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!editingDept) return;
    await DataApi.patchGroup(editingDept.id, {
      name: newDeptName.trim(),
      code: newDeptCode.trim().toUpperCase(),
      description: newDeptDesc.trim(),
      max_file_size_mb: Number(newDeptMaxSize) || 50,
  ...
  const handleDeleteDepartment = async (dept: Department) => {
    if (!window.confirm(`حذف واحد «${dept.name}»؟`)) return;
    await DataApi.deleteGroup(dept.id);
    onRefresh();
  };

  const handleCreateRole = async (e: React.FormEvent) => {
    e.preventDefault();
    await DataApi.createRole({ name: roleName, code: roleCode, description: roleDesc, permission_level: roleLevel });
    setShowAddRoleModal(false);
    setRoleName('');
    setRoleCode('');
    setRoleDesc('');
    const r = await DataApi.roles();
    setRoles((r.roles || []).map(mapRole));
  };

  const handleEditRole = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!editingRole) return;
    await DataApi.patchRole(editingRole.id, { name: roleName, description: roleDesc, permission_level: editingRole.isSystem ? undefined : roleLevel });
    setEditingRole(null);
    const r = await DataApi.roles();
    setRoles((r.roles || []).map(mapRole));
  };
  ...
  const handleDeleteRole = async (role: OrgRole) => {
    if (!window.confirm(`حذف نقش «${role.name}»؟`)) return;
    await DataApi.deleteRole(role.id);
    const r = await DataApi.roles();
    setRoles((r.roles || []).map(mapRole));
  };
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-xs space-y-4">
        ...
        <div className="flex items-center gap-2 border-t border-slate-100 pt-3">
          ...
          >
            <ShieldCheck className="w-4 h-4" />
            <span>نقش‌های سازمانی ({roles.length})</span>
          </button>
        </div>
                ...
                <div className="pt-3 border-t border-slate-100 text-[11px] text-slate-500">
                  ...
                  {isAdmin && (
                    <div className="flex gap-2 mt-3">
                      <button onClick={() => openEditDept(dept)} className="px-2 py-1 bg-slate-100 rounded-lg flex items-center gap-1 text-[11px] font-bold">
                        <Edit3 className="w-3.5 h-3.5" /> اصلاح / تغییر نام
                      </button>
                      <button onClick={() => handleDeleteDepartment(dept)} className="px-2 py-1 bg-red-50 text-red-700 rounded-lg flex items-center gap-1 text-[11px] font-bold">
      ...
      {activeTab === 'roles' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {roles.map((role) => (
            <div key={role.id} className="bg-white rounded-2xl p-5 border border-slate-200 space-y-3">
              <div className="flex items-start justify-between">
                <div>
                  <h3 className="font-bold text-sm">{role.name}</h3>
                  <p className="text-[11px] text-slate-500 mt-1">{role.description}</p>
                </div>
                <span className="text-[10px] font-mono bg-slate-100 px-2 py-0.5 rounded">{role.code}</span>
              </div>
              <p className="text-xs text-slate-600">سطح دسترسی: {StorageService.getRoleTitle(role.permissionLevel)} • اعضا: {role.membersCount}</p>
              {isAdmin && (
                <div className="flex gap-2">
                  <button
                    onClick={() => { setEditingRole(role); setRoleName(role.name); setRoleDesc(role.description); setRoleLevel(role.permissionLevel); }}
                    className="px-2 py-1 bg-slate-100 rounded-lg text-[11px] font-bold flex items-center gap-1"
                  >
                    <Edit3 className="w-3.5 h-3.5" /> ویرایش
                  </button>
                  {!role.isSystem && (
                    <button onClick={() => handleDeleteRole(role)} className="px-2 py-1 bg-red-50 text-red-700 rounded-lg text-[11px] font-bold flex items-center gap-1">
                      <Trash2 className="w-3.5 h-3.5" /> حذف
                    </button>
                  )}
                </div>
      ...
      {editingUser && (
        <div className="fixed inset-0 bg-slate-900/40 flex items-center justify-center p-4 z-50">
          <form onSubmit={handleEditUser} className="bg-white rounded-2xl max-w-md w-full p-6 space-y-3 text-xs">
            <h2 className="text-sm font-bold">ویرایش کاربر و نقش سازمانی</h2>
            <input className="w-full bg-slate-100 p-2 rounded-lg border" value={editFullName} onChange={(e) => setEditFullName(e.target.value)} />
            <input className="w-full bg-slate-100 p-2 rounded-lg border" value={editEmail} onChange={(e) => setEditEmail(e.target.value)} />
            <select className="w-full bg-slate-100 p-2 rounded-lg border" value={editRole} onChange={(e) => setEditRole(e.target.value as UserRole)}>
              <option value="user">کاربر عادی</option>
              <option value="group_admin">مدیر گروه</option>
              <option value="system_admin">مدیر سامانه</option>
              <option value="viewer">مشاهده‌گر</option>
            </select>
            <select className="w-full bg-slate-100 p-2 rounded-lg border" value={editOrgRoleId} onChange={(e) => setEditOrgRoleId(e.target.value)}>
              <option value="">نقش سیستمی پیش‌فرض</option>
              {roles.map((r) => <option key={r.id} value={r.id}>{r.name}</option>)}
            </select>
            <select className="w-full bg-slate-100 p-2 rounded-lg border" value={editDeptId} onChange={(e) => setEditDeptId(e.target.value)}>
              {departments.map((d) => <option key={d.id} value={d.id}>{d.name}</option>)}
            </select>
            <div className="flex justify-end gap-2">
              <button type="button" onClick={() => setEditingUser(null)} className="px-3 py-1.5 bg-slate-100 rounded-lg">انصراف</button>
              <button type="submit" className="px-4 py-1.5 bg-blue-600 text-white rounded-lg font-bold">ذخیره</button>
            </div>
          </form>
        </div>
      )}
      ...
      {editingDept && (
        <div className="fixed inset-0 bg-slate-900/40 flex items-center justify-center p-4 z-50">
          <form onSubmit={handleEditDepartment} className="bg-white rounded-2xl max-w-md w-full p-6 space-y-3 text-xs">
            <h2 className="text-sm font-bold">اصلاح واحد سازمانی</h2>
            <input className="w-full bg-slate-100 p-2 rounded-lg border" value={newDeptName} onChange={(e) => setNewDeptName(e.target.value)} />
            <input className="w-full bg-slate-100 p-2 rounded-lg border" value={newDeptCode} onChange={(e) => setNewDeptCode(e.target.value)} />
            <textarea className="w-full bg-slate-100 p-2 rounded-lg border" rows={2} value={newDeptDesc} onChange={(e) => setNewDeptDesc(e.target.value)} />
            <div className="flex justify-end gap-2">
              <button type="button" onClick={() => setEditingDept(null)} className="px-3 py-1.5 bg-slate-100 rounded-lg">انصراف</button>
              <button type="submit" className="px-4 py-1.5 bg-blue-600 text-white rounded-lg font-bold">ذخیره تغییرات</button>
            </div>
          </form>
        </div>
      )}
      ...
      {(showAddRoleModal || editingRole) && (
        <div className="fixed inset-0 bg-slate-900/40 flex items-center justify-center p-4 z-50">
          <form onSubmit={editingRole ? handleEditRole : handleCreateRole} className="bg-white rounded-2xl max-w-md w-full p-6 space-y-3 text-xs">
            <h2 className="text-sm font-bold">{editingRole ? 'ویرایش نقش سازمانی' : 'تعریف نقش سازمانی'}</h2>
            <input placeholder="نام نقش" className="w-full bg-slate-100 p-2 rounded-lg border" value={roleName} onChange={(e) => setRoleName(e.target.value)} required />
            {!editingRole && (
              <input placeholder="کد نقش" className="w-full bg-slate-100 p-2 rounded-lg border" value={roleCode} onChange={(e) => setRoleCode(e.target.value)} required />
            )}
            <textarea placeholder="شرح" className="w-full bg-slate-100 p-2 rounded-lg border" rows={2} value={roleDesc} onChange={(e) => setRoleDesc(e.target.value)} />
            {(!editingRole || !editingRole.isSystem) && (
              <select className="w-full bg-slate-100 p-2 rounded-lg border" value={roleLevel} onChange={(e) => setRoleLevel(e.target.value as UserRole)}>
            ...
            <div className="flex justify-end gap-2">
              <button type="button" onClick={() => { setShowAddRoleModal(false); setEditingRole(null); }} className="px-3 py-1.5 bg-slate-100 rounded-lg">انصراف</button>
              <button type="submit" className="px-4 py-1.5 bg-blue-600 text-white rounded-lg font-bold">ذخیره</button>
            </div>
          </form>
        </div>
