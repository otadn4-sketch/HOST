...
import { DataApi, downloadFile } from '../services/api';
...
export const FileRepository: React.FC<FileRepositoryProps> = ({
    ...
    return files.filter(file => {
      ...
      if (searchQuery.trim()) {
        ...
        const matches = 
          ...
          file.checksumSha256.toLowerCase().includes(q) ||
  ...
  }, [files, currentUser, searchQuery, selectedDept, selectedTopic, selectedClassification, selectedScanStatus]);
  ...
  const handleOpenFileDetails = async (file: FileItem) => {
    try {
      ...
      setSelectedFile({
        ...file,
        ...{
          viewCount: res.file.view_count,
          versions: res.file.versions,
        },
      });
    } catch {
            ...
            <table className="w-full text-right text-xs">
              <thead className="bg-slate-50 text-slate-500 border-b border-slate-200 font-medium">
                <tr>
                  ...
                  <th className="p-3.5 font-medium">پویش ClamAV</th>
      ...
      {selectedFile && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-in fade-in">
          <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto border border-slate-200 shadow-2xl p-6 space-y-5">
              ...
              <div className="p-3.5 bg-red-50 rounded-xl border border-red-200 text-xs text-red-800 flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="mt-1 text-[11px] text-red-700 font-medium">
                    فایل در مسیر ایزوله نگهداری شده و امکان دانلود برای کاربران عادی مسدود است.
                  </p>
                </div>
              </div>
            ) : (
            ...
            {/* Storage Vault & Checksum Integrity Details (Windows Server 2022 Path) */}
            <div className="space-y-2 p-4 bg-slate-50 rounded-xl border border-slate-200 text-xs">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[10px] text-slate-400">خارج از IIS / وب‌روت</span>
              </div>
              <p className="font-mono text-[11px] text-slate-800 bg-white p-2.5 rounded-lg break-all border border-slate-200">
                D:\EytanVault\Storage\Vault\{selectedFile.storedVaultName}
              </p>
