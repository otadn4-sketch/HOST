...
from app.models.entities import FileObject, FilePermission, Group, User, utcnow
...
def serialize_file(db: DBSession, file: FileObject, user: User) -> dict:
    ...
    return {
        ...
        "stored_vault_name": file.stored_name,  # uuid only, never a filesystem path
        "topic": file.topic,
        ...
        "quarantine_reason": file.quarantine_reason if user.role == "system_admin" or access.can_manage else None,
        ...
        if access.can_manage or user.role == "system_admin"
        else [],
    }
