...
def list_logs(
    ...
    for row in rows:
        ...
        if severity and severity != "all" and row.severity != severity:
            continue
        if q.strip():
            blob = " ".join([row.username, row.action_title, row.target_resource, row.details, row.ip_address]).lower()
...
def export_logs(db: DBSession = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    if not is_admin(user):
        query = query.filter(AuditLog.user_id == user.id)
    rows = query.all()
    lines = ["id,timestamp,username,role,action,target,ip,severity,details"]
    for r in rows:
